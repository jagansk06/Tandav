            ...
            override fun onConnectionStateChange(
                ...
                when (newState) {
                    BluetoothProfile.STATE_CONNECTED -> {
                        // Track every connected remote immediately so
                        // characteristic notifications can be sent without
                        // waiting for (or requiring) a bond. The Tandav
                        // characteristics are unencrypted; application-level
                        // security is handled above BLE, and the Tandav app
                        // signs every session with an HMAC shared secret.
